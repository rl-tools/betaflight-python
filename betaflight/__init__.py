...
class Simulator():
    ...
    def step(self, rpms):
        ...
        # returns position, orientation, linear_velocity, angular_velocity, accelerometer, pressure, rc_channels # [m, (w, x, y, z) quaternion, m/s, rad/s, m/s^2, ?, uS] all in FLU, RC channels: 1000-2000
        return np.zeros(3), np.array([1, 0, 0, 0]), np.zeros(3), np.zeros(3), np.zeros(3), 101325, [1500] * self.SIMULATOR_MAX_RC_CHANNELS
    ...
    async def run(self):
        ...
        while True:
            ...
            timestamp = time.time()
            position, orientation, linear_velocity, angular_velocity, accelerometer, pressure, rc_channels = self.step(rpms)
            rc_packet = struct.pack(f'<d{self.SIMULATOR_MAX_RC_CHANNELS}h', timestamp, *rc_channels)
